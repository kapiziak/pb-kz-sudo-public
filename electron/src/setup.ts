...
import {
    ...
    getImgSrcPolicy,
    getMediaSrcPolicy,
    getScriptSrcElemPolicy,
    getScriptSrcPolicy,
    getStyleSrcElemPolicy,
    getWorkerSrcPolicy,
} from "./cors-policy";
    ...
    async init(): Promise<void> {
        ...
        this.MainWindow.webContents.on("dom-ready", () => {
            ...
            session.defaultSession.webRequest.onBeforeSendHeaders(
                (details, callback) => {
                    details.requestHeaders["Referer"] = "http://sudo-app";
                    callback({
                        cancel: false,
...
export function setupContentSecurityPolicy(customScheme: string): void {
    session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
        const customSecurityPolicies: Array<string> = [
            `img-src ${getImgSrcPolicy()}`,
            `style-src-elem ${getStyleSrcElemPolicy()}`,
            `font-src ${getFontSrcPolicy()}`,
            `script-src ${getScriptSrcPolicy()}`,
            `script-src-elem ${getScriptSrcElemPolicy()}`,
            `connect-src ${getConnectSrcPolicy(customScheme)}`,
            `worker-src ${getWorkerSrcPolicy()}`,
            `media-src ${getMediaSrcPolicy()}`,
        ];

        const securityPoliciesAsString = customSecurityPolicies.join("; ");
        ...
        callback({
            responseHeaders: {
                ...
                "Content-Security-Policy": [
                    electronIsDev
                        ? `${securityPoliciesAsString}; default-src ${customScheme}://* 'unsafe-inline' devtools://* 'unsafe-eval' data:`
                        : `${securityPoliciesAsString}; default-src ${customScheme}://* 'unsafe-inline' data:`,
                ],
            },
        });
    });
